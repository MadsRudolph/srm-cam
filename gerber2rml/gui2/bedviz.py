"""The probed surface as a solid you can turn over.

The flat heatmap on the stage answers "where is it high?" faster than this ever
will, and for choosing a cut depth that is the whole question. This answers a
different one: what SHAPE is it. A board bowed along one axis, dished in the
middle, or tipped on one corner all read as much the same wash of colour from
above, and they want different fixtures — a bow wants support underneath, a tip
wants a screw moved.

The deviations are tens of microns across a hundred millimetres, so nothing is
visible at true scale. The Z exaggeration is therefore the control that
matters, and it is a slider rather than a fixed number on purpose: the honest
picture is the one where you can watch the shape hold together as you wind it
up. A feature that only appears at 800x is noise.

A copy of the first interface's window, carrying this interface's palette. The
surface itself is the height map's own.
"""
import math
import os
...
class BedVisualizerWindow(QMainWindow):
    ...
    def __init__(self, xs, ys, Z, points, title="Bed visualizer", parent=None,
        ...
        self.range_lbl.setStyleSheet(f"color:{theme.TEXT_3};")
        ...
        cl.addWidget(QLabel("Z ×", styleSheet=f"color:{theme.TEXT_3};"))
        cl.addWidget(self.exag_slider)
        cl.addWidget(QLabel("hover a point for its value",
                            styleSheet=f"color:{theme.TEXT_2};"))
        cl.addStretch(1)
