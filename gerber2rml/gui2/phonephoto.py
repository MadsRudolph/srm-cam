"""Phone hand-off: a QR code, a phone camera, and the photo lands here.

Photographing a board on the bed means a phone, and getting the picture off the
phone is the part that wastes the afternoon. Two transports, picked by the
Relay field and remembered:

- **Relay set** (the default) — phone and app both talk to a small hosted
  relay, so it works on any network, eduroam and mobile data included, because
  this PC accepts no inbound connection at all.
- **"lan"** — a one-shot local server and a QR carrying this PC's address. No
  infrastructure, but the phone and the PC have to be able to reach each other,
...
class PhonePhotoDialog(QDialog):
    def __init__(self, parent, save_dir):
        ...
        self.hint.setStyleSheet(f"color: {theme.TEXT_3};")
